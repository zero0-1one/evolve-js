...
class Evolvement {
    ...
    mutateOrNot(entity) {
        return Math.random() <= this.config['mutation'] ? this.mutate(Clone(entity)) : Clone(entity);
    }
    ...
    evolve(config, population) {
        ...
        if (!population) {
            for (let i = 0; i < this.config['size']; ++i) {
                this.population.push(Clone(this.seed()));
            }
        } else {
        ...
        for (let i = 0; i < this.config['iterations']; ++i) {
            ...
            let isFinished = (typeof r != "undefined" && !r) || (i == this.config['iterations'] - 1);
