...
class Evolvement {
  ...
  async evolve(config, population) {
    this.config = {
      'size': 100,
      'mutation': 0.2,
      'iterations': 100,
    ...
    for (let i = 0; i < this.config['iterations']; ++i) {
      ...
      if (!this.bestEver || this.optimize(stats['best'], this.bestEver.fitness)) {
        this.bestEver = { fitness: stats['best'], iteration: i, stagnant: 0 }
      } else {
        this.bestEver.stagnant++
      }

      if (!isNaN(pop[0].fitness)) {
        let mean = pop.reduce((a, b) => a + b.fitness, 0) / pop.length
        let stdev = Math.sqrt(pop.map(a => (a.fitness - mean) * (a.fitness - mean)).reduce((a, b) => a + b, 0) / pop.length)
        stats['mean'] = mean
        stats['stdev'] = stdev
      }
      ...
      if (!isFinished && this.config['stagnant'] > 0 && this.bestEver && this.bestEver.stagnant >= this.config['stagnant']) {
        isFinished = true
        console.log('停滞： ', this.bestEver, stats)
      }
      if (isFinished || this.config['skip'] == 0 || i % this.config['skip'] == 0) {
        await this.notification(pop, i, stats, isFinished)
      }

      if (isFinished) break

      let newPop = []
      if (this.config['fittestSurvive']) newPop.push(pop[0].entity)
